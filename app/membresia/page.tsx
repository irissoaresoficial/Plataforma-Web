    ...
    <div style={{ width: '100%', background: '#0A0A0C', color: '#F4F3EF', overflowX: 'hidden' }}>
      ...
      <div id="top" style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', padding: 'clamp(88px,12vh,130px) clamp(14px,3vw,36px) clamp(30px,5vh,56px)', overflow: 'hidden' }}>
        ...
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: 'radial-gradient(ellipse 75% 60% at 20% 25%,rgba(200,155,74,.16),transparent 62%),linear-gradient(to bottom,rgba(10,10,12,.5),rgba(10,10,12,.92))',
            pointerEvents: 'none',
          }}
        />
        <div style={{ position: 'relative', zIndex: 3, maxWidth: 1240, margin: '0 auto', width: '100%', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: 'clamp(28px,4vw,68px)', alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(18px,2.2vw,28px)' }}>
            <Reveal>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', color: '#C89B4A' }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#C89B4A' }} />
                Aún no ha abierto · lista de espera
              </div>
            </Reveal>
            <Reveal as="h1" delay={70} style={{ margin: 0, fontSize: 'min(clamp(38px,5.8vw,78px),15vh)', fontFamily: 'var(--serif)', fontVariationSettings: "'opsz' 144, 'SOFT' 100", lineHeight: 0.99, letterSpacing: '-.026em', maxWidth: '15ch', textWrap: 'pretty' }}>
              Entenderlo lleva una sesión. <span style={{ color: '#C89B4A' }}>Cambiarlo lleva meses.</span>
            </Reveal>
            <Reveal delay={150}>
              <p style={{ margin: 0, fontSize: 'clamp(16px,1.25vw,20px)', lineHeight: 1.55, color: 'rgba(244,243,239,.6)', maxWidth: '42ch' }}>
                  ...
                  {Array.from({ length: 12 }, (_, i) => (
                    <div
                      ...
                      style={{
                        ...
                        background: i < 3 ? 'linear-gradient(to top,#C89B4A,#E0B15C)' : 'rgba(244,243,239,.1)',
                      }}
                    />
                  ))}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 600, letterSpacing: '.04em', textTransform: 'uppercase', color: 'rgba(244,243,239,.4)' }}>
                  <span>mes 1</span>
                  <span>sin fecha de final</span>
                </div>
                <span style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(244,243,239,.55)' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', color: 'rgba(244,243,239,.5)' }}>Precio de reserva</span>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 'clamp(52px,6.5vw,80px)', fontFamily: 'var(--serif)', fontVariationSettings: "'opsz' 144, 'SOFT' 100", letterSpacing: '-.032em', lineHeight: 0.85, color: '#C89B4A' }}>
                {eur(MEMBRESIA.precioReserva)}
              </span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, paddingBottom: 8 }}>
                <span style={{ fontSize: 18, fontWeight: 500, color: 'rgba(244,243,239,.4)', textDecoration: 'line-through' }}>{eur(MEMBRESIA.precio)}</span>
                <span style={{ fontSize: 13, color: 'rgba(244,243,239,.5)' }}>al mes</span>
              </div>
            </div>

            <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6, color: 'rgba(244,243,239,.58)' }}>
      ...
      <div style={{ position: 'relative', zIndex: 3, background: '#F5F4F0', color: '#0A0A0C', padding: 'clamp(60px,8vw,120px) clamp(14px,3vw,36px)' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 'clamp(24px,3vw,40px)' }}>
            ...
            {MEMBRESIA.incluye.map((item, i) => (
              <Reveal key={i} delay={i * 80}>
                <div
                  ...
                  style={{
                    background: '#FFFFFF',
                    border: `1px ${item === PENDIENTE ? 'dashed' : 'solid'} rgba(10,10,12,.14)`,
                    borderRadius: 18,
                    padding: 24,
                    display: 'flex',
                    flexDirection: 'column',
                ...
                >
                  <span data-cardnum style={{ fontSize: 30, fontWeight: 900, letterSpacing: '-.022em', color: 'rgba(10,10,12,.14)', transition: 'color .5s ease' }}>
          ...
          <Reveal>
            <p style={{ margin: 0, fontSize: 'clamp(15px,1.15vw,18px)', lineHeight: 1.6, color: '#5C5972', maxWidth: '52ch' }}>
      ...
      {/* PARA QUIÉN */}
      <div style={{ position: 'relative', zIndex: 3, background: '#0A0A0C', padding: 'clamp(60px,8vw,120px) clamp(14px,3vw,36px)' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 'clamp(24px,3.4vw,56px)' }}>
          <Reveal style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', color: '#C89B4A' }}>Para quién sí</span>
            {[
              ...
              <div key={l} style={{ display: 'flex', gap: 12, fontSize: 16, lineHeight: 1.55 }}>
                ...
                <span style={{ color: 'rgba(244,243,239,.72)' }}>{l}</span>
              </div>
            ))}
          </Reveal>
          <Reveal delay={100} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', color: 'rgba(244,243,239,.42)' }}>Para quién no</span>
            {[
              ...
              <div key={l} style={{ display: 'flex', gap: 12, fontSize: 16, lineHeight: 1.55 }}>
                <span style={{ color: 'rgba(244,243,239,.32)' }}>—</span>
                <span style={{ color: 'rgba(244,243,239,.5)' }}>{l}</span>
              </div>
            ))}
          </Reveal>
        </div>
      ...
      <div style={{ position: 'relative', zIndex: 3, background: '#0A0A0C', padding: '0 clamp(14px,3vw,36px) clamp(60px,8vw,110px)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 20 }}>
          <Reveal>
            <div style={{ fontSize: 'clamp(26px,3.6vw,48px)', fontFamily: 'var(--serif)', fontVariationSettings: "'opsz' 144, 'SOFT' 100", lineHeight: 1.04, letterSpacing: '-.022em', maxWidth: '18ch', textWrap: 'pretty' }}>
          ...
          <Reveal delay={140}>
            <span style={{ fontSize: 13, color: 'rgba(244,243,239,.4)' }}>No se cobra nada hoy.</span>
          </Reveal>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'flex', justifyContent: 'space-between', gap: 18, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14 }}>
              ...
              <Link href="/cursos" style={{ fontSize: 13, color: 'rgba(244,243,239,.7)' }}>Cursos</Link>
          ...
          <span style={{ fontSize: 11, color: 'rgba(244,243,239,.28)' }}>© 2026 · La comunidad</span>
        </div>
      </div>
    </div>
  );
